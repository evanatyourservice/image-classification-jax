...
class ResNet(nn.Module):
    ...
    block_cls: ModuleDef
    num_classes: int
    first_layer_kernel_size: int = 3
    first_layer_stride: int = 1
    first_layer_max_pool: bool = False
    num_filters: int = 64
    ...
    def __call__(self, x, is_training: bool = True):
        ...
        x = conv(
            self.num_filters,
            (self.first_layer_kernel_size, self.first_layer_kernel_size),
            (self.first_layer_stride, self.first_layer_stride),
            padding="SAME",
            name="conv_init",
        )(x)
        x = norm(name="bn_init")(x)
        x = self.act(x)
        if self.first_layer_max_pool:
        ...
        x = nn.Dense(
            self.num_classes, dtype=self.dtype, kernel_init=nn.initializers.he_normal()
        )(x)
        x = jnp.asarray(x, self.dtype)
        return x


ResNetTiny = partial(ResNet, stage_sizes=[1, 1, 1, 1], block_cls=ResNetBlock)
